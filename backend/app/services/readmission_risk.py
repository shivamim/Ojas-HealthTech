def predict_readmission_risk(patient) -> str:
    risk = 0
    ...
    missed = sum(1 for c in patient.checkins if c.status == "MISSED")
    if missed > 2: risk += 30

    open_escalations = sum(1 for e in patient.escalations if e.status == "OPEN")
